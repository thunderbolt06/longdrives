...
// Typeahead suggestions: Google Places Text Search when a key is set
// (returns coords in one call), Nominatim otherwise.
  ...
  if (gkey) {
    try {
      ...
      return (data.places || []).map((p) => {
        ...
        return {
          name: addr.startsWith(title) ? addr : [title, addr].filter(Boolean).join(", "),
...
// One-to-many traffic-aware drive durations via Google Route Matrix.
async function googleMatrix(origin, targets) {
  const wp = (p) => ({
    waypoint: { location: { latLng: { latitude: p.lat, longitude: p.lon } } },
  });
  const data = await fetchJson(`${ROUTES_API}/distanceMatrix/v2:computeRouteMatrix`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
  ...
  for (const e of data) {
    if (e.condition === "ROUTE_EXISTS") out[e.destinationIndex] = parseFloat(e.duration);
  }
  return out;
}

// Traffic-aware route through waypoints via Google Routes API.
async function googleRoute(points) {
  const loc = (p) => ({ location: { latLng: { latitude: p.lat, longitude: p.lon } } });
  const data = await fetchJson(`${ROUTES_API}/directions/v2:computeRoutes`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      "X-Goog-Api-Key": googleKey(),
  ...
  if (!r) throw new Error("Google Routes returned no route");
  return {
...
// Standard Google encoded-polyline decoder → [lon, lat] pairs (OSRM order).
...
// Unified routing: Google (live traffic) when a key is set, OSRM otherwise.
// A Google failure (quota, outage) degrades to OSRM rather than erroring.
export async function routeMatrix(origin, targets) {
  if (googleKey()) {
    try {
      return await googleMatrix(origin, targets);
    } catch {}
  }
  return osrmTable(origin, targets);
}
